...
if (DEFAULT_NETWORK_ID === 0) {
    ...
    NETWORK_ID = parseInt(process.env.VUE_APP_TEST_AVALANCHE_JS_NETWORKID as string);
    CHAIN_ID = process.env.VUE_APP_TEST_AVALANCHE_JS_CHAINID as string;
}

let avalanche = new avalancheJS.Avalanche(IP, PORT, PROTOCOL, NETWORK_ID, CHAIN_ID);
